...
fnpred <- function(mod,lmvo,newdata){
  ...
  if (!((length(unique(mod$data$z))==1)|(length(unique(mod$data$z))==nrow(mod$data)))) {
    ...
    rfo <- rfsrc(Y~.,data= data.frame(Y = lmvo$obj.varGuid$residuals, #lmvo$obj.varGuid$model[,1] ,
                      X = subset(lmvo$obj.varGuid$model, select = -c(1,ncol(lmvo$obj.varGuid$model)))) )
